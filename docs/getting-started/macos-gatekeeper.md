...
So the first launch takes one extra step.
...
What you are being told is that Apple has not checked this app, which is true.
Downloading it from [the releases page](https://github.com/akaion-ai/annona/releases)
and building it yourself from the same tag are the two ways to know what it is;
the source is the whole repository.
...
`notarytool` in the release workflow. It is on the list. Until then the extra
click is the honest price of an unsigned beta, and it is the same one every small
macOS project charges.
